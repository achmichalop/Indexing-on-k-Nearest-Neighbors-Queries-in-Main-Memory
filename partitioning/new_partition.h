            ...
            void PartitionUniform(const Relation& R, Relation *pR, size_t *pR_sizes, int runNumPartitionsPerRelation)
            {
                ...
                for (int i = 0; i < runNumPartitions; i++) {
                    ...
                    for (int j = 0; j < 16; j += 4) {
                        ...
                        // }
                        pR_sizes[i*16+j+3] = pR_sizes[i*16+j + 2] + pR_sizes[i*16+j + 1] + pR_sizes[i*16+j] + counter;
                        pR_sizes[i*16+j+2] = pR_sizes[i*16+j+1] + pR_sizes[i*16+j] + counter;
                        pR_sizes[i*16+j+1] = pR_sizes[i*16+j] + counter;
                        pR_sizes[i*16+j] = counter;

                        counter = pR_sizes[i*16+j+3]; 
                    }
                    pR[i].resize(counter);

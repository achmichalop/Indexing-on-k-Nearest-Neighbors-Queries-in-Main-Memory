...
namespace twoLevel{ 
    ...
    namespace window
    {
        ...
        inline void InternalLoop_Range_Corners_A(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_A(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_A(r,S,lastR,resultItemsIds);
        };

        inline void InternalLoop_Range_Corners_AB(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_AB(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_AB(r,S,lastR,resultItemsIds);
        };


        inline void InternalLoop_Range_Corners_AC(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_AC(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_AC(r,S,lastR,resultItemsIds);
        };


        inline void InternalLoop_Range_Corners_ABCD(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_ABCD(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_ABCD(r,S,lastR,resultItemsIds);
        };
        ...
        inline void InternalLoop_Range_Border_A_Horizontally(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {    
        ...
        inline void Range_Border_A_Horizontally(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Border_A_Horizontally(r,S,lastR,resultItemsIds);
        };


        inline void InternalLoop_Range_Borders_AB(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {    
        ...
        inline void Range_Borders_AB(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Borders_AB(r,S,lastR,resultItemsIds);
        };
        ...
        inline void InternalLoop_Range_Border_A_Vertically(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        { 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Border_A_Vertically(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Border_A_Vertically(r,S,lastR,resultItemsIds);

        };


        inline void InternalLoop_Range_Borders_AC(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds)
        { 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Borders_AC(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Borders_AC(r,S,lastR,resultItemsIds);
    ...
    namespace disk
    {
        ...
        inline void InternalLoop_Range_Corners_A(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_A(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_A(r,S,lastR,resultItemsIds, epsilon, point);
        };

        inline void InternalLoop_Range_Corners_AB(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_AB(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_AB(r,S,lastR,resultItemsIds, epsilon, point);
        };


        inline void InternalLoop_Range_Corners_AC(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_AC(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_AC(r,S,lastR,resultItemsIds, epsilon, point);
        };


        inline void InternalLoop_Range_Corners_ABCD(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Corners_ABCD(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Corners_ABCD(r,S,lastR,resultItemsIds, epsilon, point);
        };


        inline void InternalLoop_Range_Border_A_Horizontally(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {    
        ...
        inline void Range_Border_A_Horizontally(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Border_A_Horizontally(r,S,lastR,resultItemsIds, epsilon, point);
        };


        inline void InternalLoop_Range_Borders_AB(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {                 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {    
        ...
        inline void Range_Borders_AB(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Borders_AB(r,S,lastR,resultItemsIds, epsilon, point);
        };


        inline void InternalLoop_Range_Border_A_Vertically(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        { 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Border_A_Vertically(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Border_A_Vertically(r,S,lastR,resultItemsIds, epsilon, point);

        };


        inline void InternalLoop_Range_Borders_AC(RelationIterator firstFS,Record &S, RelationIterator lastFS,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        { 
            auto pivot = firstFS;
            while ((pivot < lastFS))
            {
        ...
        inline void Range_Borders_AC(Relation &pR, Record &S, size_t startR, size_t endR,vector<RecordId> &resultItemsIds, Coord epsilon, Coordinates point)
        {
            auto r = pR.begin() + startR;
            auto lastR = pR.begin() + endR;

            InternalLoop_Range_Borders_AC(r,S,lastR,resultItemsIds, epsilon, point);
